...
function statusClass(status){const s=txt(status,'').toUpperCase();if(s.includes('DESL'))return 'parada';if(s.includes('MANUT'))return 'manutencao';if(s.includes('LIG'))return 'ligada';return 'parada'}
function statusLabel(status){const s=txt(status,'PARADA').toUpperCase();if(s.includes('DESL'))return 'DESLIGADA';if(s.includes('MANUT'))return 'MANUTENÇÃO';if(s.includes('LIG'))return 'LIGADA';return 'PARADA'}
...
function renderEmpacotadeira(emp){
  const rawStatus = emp.Status || emp.status || 'DESLIGADA';
  const isOn = boolOn(rawStatus);
  const status = isOn ? 'LIGADA' : 'DESLIGADA';
